...
// 	$id = $_GET['id'];
// }
$sql ="SELECT `date`, `blog_title`, `blog_content`, `banner_img`, `categories` FROM `post_tab` WHERE `userid` = '".$_SESSION['email']."' AND `id` = '".$_SESSION['id']."'";
 $res = mysqli_query($conn,$sql);
 if(mysqli_num_rows($res) > 0 ){
	while($row=mysqli_fetch_assoc($res)){
    // echo $row['date'];
    // $_SESSIOON['date']= $row['date'];
     ?>
...
?>
              <!-- fetch the all records from database close -->
